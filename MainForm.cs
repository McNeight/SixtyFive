...
#region Run control
		...
		{
			if (dasmview.Visible == true) dasmview.runupdate();
			if (cpuview.Visible == true) cpuview.update();
			if (memview.Visible == true) memview.update();
		}
...
private void miSpeed_Select(object sender, System.EventArgs e)
	{
		...
		for (int i=0; i<cmSpeed.MenuItems.Count; i++) cmSpeed.MenuItems[i].Checked = false;
...
#region Menu handlers
		...
		private void miFill_Click(object sender, System.EventArgs e)
		{
			...
			if (!ri.cancel) machine.mem.FillMem(ri.start,ri.end,ri.data);
		...
		private void miMemGoto_Click(object sender, System.EventArgs e)
		{
			...
			if (memview.Focused == false && dasmview.Focused == false) return;
			...
			if (memview.Focused == true) memview.SetCurrentAddress(addr);
			if (dasmview.Focused == true) dasmview.SetCurrentAddress(addr);
