...
namespace sim6502
{
	...
	{	
		...
		public void StartAddress(ushort addr) 
		{
			curAddress = addr;
		}
		...
		public bool readBinaryRAW() 
		{
			...
			if (type == FileType.BinaryORG) curAddress = readLSBMSB();
			...
			{	
				...
				if (curAddress == 0xFFFF) overflow = true;
				curAddress++;
			}
			return true;
		}
		...
		public ushort readLSBMSB() 
		{
			...
			try 
			{
				d1 = (ushort)br.ReadByte();
				a = (ushort)((ushort)br.ReadByte() * 256);
				a = (ushort)(a + d1);
			}
			catch 
			{
		...
		public bool LoadFile(string file,FileType typ,ushort start) 
		{
			bool pass = true;
